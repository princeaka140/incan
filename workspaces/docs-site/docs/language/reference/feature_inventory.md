...
The toolchain install path is manifest-driven, checksum-verified, provisions the Rust backend where the channel can safely do so, and installs `incan` plus `incan-lsp` from the same release archives across direct and package-manager channels. Direct, npm, and pipx installation use the shared installer to provision stable Rust and `wasm32-wasip1` unless explicitly skipped; Homebrew installs the prebuilt command binaries and leaves Rust management to the user.
