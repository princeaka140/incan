...
- **Compiler/Codegen**: `enumerate(...)` now emits Incan `int` (`i64`) indices instead of leaking Rust `usize` values through tuple-unpacked loop bindings, so indices can flow into ordinary `int` fields and helper parameters without manual casts.
- **Compiler/Codegen**: `list[str].append("...")` now materializes owned `String` elements instead of cloning borrowed Rust `&str` literals, fixing downstream aggregate/projection builders that append literal output column names (#391).
