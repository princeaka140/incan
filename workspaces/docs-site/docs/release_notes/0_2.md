...
- **Compiler**: `rust-metadata` is now caller-owned again instead of ambient in `TypeChecker`, which fixes the
  accidental repo-root extraction regression in plain semantic/test paths and restores cheap permissive handling for
  unextracted Rust members.
- **Compiler**: project-local rust-metadata caches now persist extracted RFC 041 item metadata under
  `target/incan_lock`, so generated metadata workspaces can reuse extracted items across sessions instead of rebuilding
  every lookup from scratch.
- **Compiler**: Borrowed Rust free-function signatures now preserve `&T` / `&mut T` semantics through more of the
  typechecker, lowering, and emission pipeline, including generated-lock metadata workspaces and typed imported
  callees. Some downstream rust-metadata dependency-loading gaps are still being closed (#257).
