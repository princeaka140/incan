...
- **Compiler**: Imported Rust free functions whose borrowed parameter signatures are available to the compiler now
  preserve borrow semantics through lowering, so generated Rust call sites emit `&value` / `&mut value` instead of
  incorrectly passing owned values (#257).
