...
- **Homebrew, npm, and pip adapters**: Homebrew formula generation, `incan` npm packaging, and `incan` pip/pipx packaging are thin adapters over the same toolchain manifest and archive contract. They are install channels, not separate compiler builds (#428).
