...
- **Semantic inspection**: `incan inspect codegraph` exports compiler-backed JSONL records for source files, modules, declarations, imports, exports, syntax-level references and calls, diagnostics, spans, provenance, and degraded-state facts (#666, #573, [RFC 102], [RFC 106]).
- **Performance and observability**: Test/build preheat phases now report visible timing, cache hits or misses, and Cargo target information so long dependency work is measurable instead of silent (#707, #723, #697).
...
- The 0.4 codegraph export emits Incan-language facts only. Resolved reference targets, resolved call targets, first-class Rust records, MCP tools, task-ranked context packs, process-risk signals, and Architect findings are follow-up RFC 106 work.
