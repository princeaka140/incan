...
- **Semantic inspection**: a compiler-backed codegraph export for Incan-language files, modules, declarations, imports, exports, body-level reference and call syntax, diagnostics, spans, provenance, language tags, and degraded-state facts.
...
- **Codegraph inspection**: `incan inspect codegraph` exports deterministic JSONL records for Incan-language source files, modules, top-level declarations, imports, public exports, body-level reference and call syntax, containment, spans, provenance, degraded state, and diagnostics. This is the concrete 0.4 codegraph slice for tooling and agent context, distinct from runtime `std.graph` and intentionally smaller than a resolved reference/call graph, MCP service, Architect recommendation engine, or first-class Rust graph.
