...
- `reference`: source-level name references inside declaration bodies, including identifier, field, `self`, and surface-path forms. `target_id` points at a declaration record for compiler-proven source identifiers, including local declarations and supported source imports. Unsupported, ambiguous, degraded, field, `self`, surface-path, and syntax-only cases keep `target_id: null`.
- `call`: source-level call expressions inside declaration bodies, including function, method, constructor, and surface-symbol calls. `target_id` points at a declaration record for compiler-proven source function and constructor calls, including local declarations, supported source imports, and facade reexports where the original declaration identity is known. Unsupported, ambiguous, degraded, method, surface-symbol, and syntax-only cases keep `target_id: null`.
